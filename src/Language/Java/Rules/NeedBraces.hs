...
module Language.Java.Rules.NeedBraces where
...
checkStatements (methodName, methodBody) path = do
  ...
  where
    ...
    checkStatement (IfThen {}) = return (methodDiagnostic methodName "A IfThen-Part contains no braces." path)
    checkStatement (IfThenElse _ _ (StmtBlock _) (StmtBlock _)) = mzero
    checkStatement (IfThenElse _ _ _ (StmtBlock _)) = return (methodDiagnostic methodName "A IfThenElse-Part contains no braces." path)
    checkStatement (IfThenElse _ _ (StmtBlock _) _) = return (methodDiagnostic methodName "A IfThenElse-Part contains no braces." path)
