...
check cUnit path = do
  ...
  checkExp exp
  where
    checkExp assign@(Assign span (NameLhs name1) EqualA (BinOp (ExpName name2) op (Lit _ (Int 1)))) =
      if eq IgnoreSourceSpan name1 name2
        then message assign (opToPostIncDec op name1) span path
        else mzero
    checkExp assign@(Assign span (NameLhs name1) EqualA (BinOp (Lit _ (Int 1)) op (ExpName name2))) =
      if eq IgnoreSourceSpan name1 name2
        then message assign (opToPostIncDec op name1) span path
        else mzero
    checkExp assign@(Assign span (NameLhs name) op (Lit _ (Int 1))) =
